...
__attribute__((constructor)) static void init_extract_gpuinfo_panthor(void) {
  register_gpu_vendor(&gpu_vendor_panthor);
}

bool gpuinfo_panthor_init(void) { return mali_init_drm_funcs(&drmFuncs, &mali_state); }

void gpuinfo_panthor_shutdown(void) { mali_shutdown_common(&mali_state, &drmFuncs); }

static const char *gpuinfo_panthor_last_error_string(void) {
  static char driver_msg[MAX_ERR_STRING_LEN] = {0};

  return mali_common_last_error_string(&mali_state, gpu_vendor_panthor.name, driver_msg);
}

static void panthor_check_fdinfo_keys(bool *is_engine, bool *is_cycles, bool *is_maxfreq, bool *is_curfreq,
...
static bool parse_drm_fdinfo_panthor(struct gpu_info *info, FILE *fdinfo_file, struct gpu_process *process_info) {
  ...
  if (!mali_common_parse_drm_fdinfo(info, fdinfo_file, process_info, dynamic_info, panthor_check_fdinfo_keys, &res))
    return false;
...
static bool gpuinfo_panthor_get_device_handles(struct list_head *devices, unsigned *count) {
  return mali_common_get_device_handles(&mali_state, &drmFuncs, &gpu_vendor_panthor, parse_drm_fdinfo_panthor, devices,
                                        count, NULL, MALI_PANTHOR);
}
...
void gpuinfo_panthor_populate_static_info(struct gpu_info *_gpu_info) {
  ...
  if (ret) {
    ...
    snprintf(static_info->device_name, sizeof(static_info->device_name), "Unknown Panthor %x", gpu_dev_info.gpu_id);
